...
def test_normalize_plain_string():
    ...
    assert result == [{"file": "outfit.jpg", "role": "costume detail", "use_as_reference": False}]


def test_normalize_dict_passthrough():
    result = _normalize_outfit_images([{"file": "a.jpg", "role": "full body"}])
    assert result == [{"file": "a.jpg", "role": "full body", "use_as_reference": False}]


def test_normalize_dict_preserves_use_as_reference():
    result = _normalize_outfit_images([{"file": "a.jpg", "role": "full body", "use_as_reference": True}])
    assert result == [{"file": "a.jpg", "role": "full body", "use_as_reference": True}]


def test_normalize_mixed():
    result = _normalize_outfit_images(["a.jpg", {"file": "b.jpg", "role": "portrait"}])
    assert result[0] == {"file": "a.jpg", "role": "costume detail", "use_as_reference": False}
    assert result[1] == {"file": "b.jpg", "role": "portrait", "use_as_reference": False}
...
def test_normalize_skips_falsy():
    assert _normalize_outfit_images(["", None, "a.jpg"]) == [
        {"file": "a.jpg", "role": "costume detail", "use_as_reference": False}
    ]
...
def test_define_with_reference_images():
    ...
    assert refs == [{"file": "dress.jpg", "role": "costume detail", "use_as_reference": False}]
...
def test_define_preserves_existing_reference_images():
    reg = OutfitRegistry(outfits={
        "dress": {"name": "Dress", "description": "", "reference_images": [
            {"file": "existing.jpg", "role": "costume detail", "use_as_reference": False}
        ]}
    })
    ...
    assert refs == [{"file": "existing.jpg", "role": "costume detail", "use_as_reference": False}]


def test_define_replaces_reference_images_when_provided():
    reg = OutfitRegistry(outfits={
        "dress": {"name": "Dress", "description": "", "reference_images": [
            {"file": "old.jpg", "role": "costume detail", "use_as_reference": False}
        ]}
    })
    r = reg.define("dress", name="Dress", description="", reference_images=["new.jpg"])
    refs = r.outfits["dress"]["reference_images"]
    assert refs == [{"file": "new.jpg", "role": "costume detail", "use_as_reference": False}]
...
def test_from_dict_normalizes_plain_string_refs():
    ...
    assert reg.outfits["jacket"]["reference_images"] == [
        {"file": "jacket.jpg", "role": "costume detail", "use_as_reference": False}
    ]
...
def test_round_trip(tmp_path):
    ...
    reg = reg.define("coat", name="Winter Coat", description="Heavy wool coat.",
                     reference_images=[{"file": "coat.jpg", "role": "full body", "use_as_reference": True}])
    save_outfit_registry(reg, path, backup=False)
    loaded = load_outfit_registry(path)
    refs = loaded.outfits["coat"]["reference_images"]
    assert refs == [{"file": "coat.jpg", "role": "full body", "use_as_reference": True}]
